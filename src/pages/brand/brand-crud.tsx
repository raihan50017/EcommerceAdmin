...
import { BrandType, GetBrandById } from '@/actions/brand/brand-action';
import BrandForm from './brand-form';

export default function BrandCRUD() {
    const axios = useAxiosInstance();
    const [brand, setBrand] = React.useState<BrandType | undefined>();
    ...
    React.useEffect(() => {
        async function getData() {
            try {
                ...
                await GetBrandById(axios, Number(id))
                    .then((res) => {
                        if (res.errors) {
    ...
    if (pageAction === PageAction.view) {
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10 ">
                <h1 className="font-bold text-xl text-left w-full mb-2">Brand</h1>
                <BrandForm data={brand} pageAction={PageAction.view} />
            </div>
        );
        ...
        return (
            <>
                ...
                <div className="flex flex-col gap-10">
                    <BrandForm data={brand} pageAction={PageAction.add} />
                </div>
            </>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <BrandForm data={brand} pageAction={PageAction.edit} />
            </div>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <BrandForm data={brand} pageAction={PageAction.delete} />
            </div>
        );
    } else {
        return (
