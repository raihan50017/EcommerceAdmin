...
import { GetSizeById, SizeType } from '@/actions/size/size-action';
import SizeForm from './size-form';

export default function SizeCRUD() {
    const axios = useAxiosInstance();
    const [size, setSize] = React.useState<SizeType | undefined>();
    ...
    React.useEffect(() => {
        async function getData() {
            try {
                ...
                await GetSizeById(axios, Number(id))
                    .then((res) => {
                        if (res.errors) {
    ...
    if (pageAction === PageAction.view) {
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10 ">
                <h1 className="font-bold text-xl text-left w-full mb-2">Size</h1>
                <SizeForm data={size} pageAction={PageAction.view} />
            </div>
        );
    } else if (pageAction === PageAction.add) {
        return (
            <>
                <BreadcrumbAddNew pageName="New Size" isShowAddNewButton={false} handleNavigateToAddNewPage={() => { }} />
                <div className="flex flex-col gap-10">
                    <SizeForm data={size} pageAction={PageAction.add} />
                </div>
            </>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <SizeForm data={size} pageAction={PageAction.edit} />
            </div>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <SizeForm data={size} pageAction={PageAction.delete} />
            </div>
        );
    } else {
        return (
