...
import { GetProjectById, Project } from '@/actions/accounting/project-action';
    ...
    React.useEffect(() => {
        async function getData() {
            try {
                ...
                await GetProjectById(axios, Number(id))
                    .then((res) => {
                        if (res.IsError) {
    ...
    if (pageAction === PageAction.view) {
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10 ">
                <h1 className="font-bold text-xl text-left w-full mb-2">Project</h1>
                <ProjectForm data={project} pageAction={PageAction.view} />
            </div>
        );
    } else if (pageAction === PageAction.add) {
        return (
            <>
                <BreadcrumbAddNew pageName="New Project" isShowAddNewButton={false} handleNavigateToAddNewPage={() => { }} />
                <div className="flex flex-col gap-10">
                    <ProjectForm data={project} pageAction={PageAction.add} />
                </div>
            </>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <ProjectForm data={project} pageAction={PageAction.edit} />
            </div>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <ProjectForm data={project} pageAction={PageAction.delete} />
            </div>
        );
    } else {
        return (
