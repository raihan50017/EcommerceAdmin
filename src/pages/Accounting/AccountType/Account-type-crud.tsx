...
import { AccountTypeType, GetAccountTypeById } from '@/actions/accounting/account-type-action';

export default function AccountTypeCRUD() {
    const axios = useAxiosInstance();
    const [accountType, setAccountType] = React.useState<AccountTypeType | undefined>();
    ...
    React.useEffect(() => {
        async function getData() {
            try {
                ...
                await GetAccountTypeById(axios, Number(id))
                    .then((res) => {
                        if (res.IsError) {
    ...
    if (pageAction === PageAction.view) {
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10 ">
                ...
                <AccountTypeForm data={accountType} pageAction={PageAction.view} />
            </div>
        );
    } else if (pageAction === PageAction.add) {
        return (
            <>
                <BreadcrumbAddNew pageName="New Account Type" isShowAddNewButton={false} handleNavigateToAddNewPage={() => { }} />
                <div className="flex flex-col gap-10">
                    <AccountTypeForm data={accountType} pageAction={PageAction.add} />
                </div>
            </>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <AccountTypeForm data={accountType} pageAction={PageAction.edit} />
            </div>
        );
        ...
        return (
            <div className="w-full flex flex-col justify-center items-center mt-2 mb-10">
                ...
                <AccountTypeForm data={accountType} pageAction={PageAction.delete} />
            </div>
        );
    } else {
        return (
